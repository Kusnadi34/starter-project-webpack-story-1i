    ...
    container.innerHTML = stories.map((s, i) => `
      <div class="story-card" data-idx="${i}" data-lat="${s.lat || ''}" data-lon="${s.lon || ''}">
        ...
        <div class="content">
          ...
          <button class="save-local" data-id="${s.id}">Simpan</button>
          <button class="delete-local" data-id="${s.id}">Hapus</button>
        </div>
      </div>
    `).join('');
    
    const saveButtons = container.querySelectorAll('.save-local');
    saveButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            e.stopPropagation();
            this.presenter.saveStoryToDb(btn.dataset.id);
        });
    });
    
    document.querySelectorAll('.story-card').forEach((card, idx) => {
        card.addEventListener('click', (e) => {
            if (e.target.classList.contains('delete-local') || e.target.classList.contains('save-local')) return;
